...
public class Voisinage{
    ...
    public Voisinage(Coords[] regle, TableauDynamiqueND grille, int ... index){
        ...
        TableauDynamiqueND tmpgrille = grille.clone();
        Boolean oob = false;
        for(Coords e : regle){
            for(int i = 0; i<n; i++){
                oob = false;
                tmp[i] = index[i];
                if(tmp[i] + e.getCoords()[i] < 0 || tmp[i] + e.getCoords()[i] >= tmpgrille.getTaille()){ // WARNING A CHANGER
                ...
                if(tmpgrille.getTab()[tmp[i]] == null){
                    this.voisinage.add(new Cellule(false));
                    oob = true;
                    break;
                }
