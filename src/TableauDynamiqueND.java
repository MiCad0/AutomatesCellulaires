...
public class TableauDynamiqueND {
    ...
    public static void main(String[] args){
        ...
        RegleDuJeu regleDuJeu = new RegleDuJeu(new int[]{3}, new int[]{2, 3});
        System.out.println();
        System.out.println();
        System.out.println();
        tab.display();
        ...
        for(int n = 0; n<30; ++n){
			for(int i = 0; i < 5; i++){
				for(int j = 0; j < 5; j++){
					if(tab.getState(j,i)){
						if(!regleDuJeu.estSurvie(tab.voisinage(regle,i,j).getVoisinage().toArray(new Cellule[0]))){
							tmp.changeState(j,i);
						}
					}
					else{
						if(regleDuJeu.estNee(tab.voisinage(regle,i,j).getVoisinage().toArray(new Cellule[0]))){
							tmp.changeState(j,i);
						}
					}
				}
